...
description: Open Linear issues as Rox workspaces with a custom script
---
...
**Work on issue → Custom script** creates a Rox workspace for the issue —
...
1. `rox workspaces create` — creates a local workspace on the issue's
   branch and launches an agent with the issue as its prompt.
2. `rox workspaces open` — opens that workspace in the desktop app via its
   deep link.

## Prerequisites

- The [Rox CLI](/docs/cli/getting-started) installed and signed in
  (`rox auth login`).
- The Rox desktop app installed (the script opens workspaces in it).
...
Save this as `~/.linear/open-in-rox.sh` and replace `PROJECT` with the ID
from above:
...
<Callout type="info">
The script depends on `rox` being on your `PATH` and `jq` being installed.
If `rox` isn't on your `PATH`, use its full path (the desktop app installs a
shim at `~/.rox/bin/rox`).
</Callout>
...
Linear runs the script, and the workspace opens in Rox with the agent
...
- **Multiple repos.** The project ID is fixed in the script. To route different
  Linear teams or projects to different Rox projects, branch on a
