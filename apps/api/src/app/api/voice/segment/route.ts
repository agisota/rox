 ...
 *   4. inserts one row into `live_transcript_segments` (the SAME table Phase-1
 *      `voice.transcribeChunk` persists to) with `created_by` = the speaker's
 *      LiveKit identity (=== better-auth user id);
 *   5. echoes `{ id }` so the worker's fan-out can dedupe on the durable id.
 *
 ...
 *   400 malformed / invalid   503 not configured
 */
