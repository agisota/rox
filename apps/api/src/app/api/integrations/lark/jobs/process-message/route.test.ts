import { beforeEach, describe, expect, mock, test } from "bun:test";

let verifyResult = true;
let verifyError: unknown = null;
const verifyMock = mock(async () => {
	if (verifyError) throw verifyError;
	return verifyResult;
});

mock.module("@/env", () => ({
	env: {
		QSTASH_CURRENT_SIGNING_KEY: "current",
		QSTASH_NEXT_SIGNING_KEY: "next",
		NEXT_PUBLIC_API_URL: "http://localhost",
	},
}));

mock.module("@upstash/qstash", () => ({
	Client: class {},
	Receiver: class {
		verify = verifyMock;
	},
}));

const processLarkMessageMock = mock(async () => ({
	success: true,
	replied: true,
	messagesSent: 1,
}));
mock.module("../../process-message", () => ({
	processLarkMessage: processLarkMessageMock,
}));

const { POST } = await import("./route");

const VALID_PAYLOAD = {
	connectionId: "conn-1",
	chatId: "oc_chat",
	messageId: "om_source",
	eventId: "evt-1",
	text: "hello",
};

function buildRequest(body: unknown, signature: string | null = "sig") {
	const headers: Record<string, string> = {
		"content-type": "application/json",
	};
	if (signature) headers["upstash-signature"] = signature;
	return new Request(
		"http://localhost/api/integrations/lark/jobs/process-message",
		{
			method: "POST",
			headers,
			body: JSON.stringify(body),
		},
	);
}

describe("lark process-message job route", () => {
	beforeEach(() => {
		verifyResult = true;
		verifyError = null;
		verifyMock.mockClear();
		processLarkMessageMock.mockClear();
	});

	test("requires an Upstash signature", async () => {
		const response = await POST(buildRequest(VALID_PAYLOAD, null));
		expect(response.status).toBe(401);
	});

	test("rejects an invalid Upstash signature", async () => {
		verifyResult = false;
		const response = await POST(buildRequest(VALID_PAYLOAD));
		expect(response.status).toBe(401);
	});

	test("rejects when Upstash signature verification throws", async () => {
		verifyError = new Error("bad signature");
		const response = await POST(buildRequest(VALID_PAYLOAD));
		expect(response.status).toBe(401);
		expect(processLarkMessageMock).not.toHaveBeenCalled();
	});

	test("processes a valid signed payload", async () => {
		const response = await POST(buildRequest(VALID_PAYLOAD));

		expect(response.status).toBe(200);
		expect(await response.json()).toEqual({
			success: true,
			replied: true,
			messagesSent: 1,
		});
		expect(verifyMock).toHaveBeenCalledWith({
			body: JSON.stringify(VALID_PAYLOAD),
			signature: "sig",
			url: "http://localhost/api/integrations/lark/jobs/process-message",
		});
		expect(processLarkMessageMock).toHaveBeenCalledWith(VALID_PAYLOAD);
	});

	test("accepts a payload with a null message_id (DM fallback)", async () => {
		const response = await POST(
			buildRequest({ ...VALID_PAYLOAD, messageId: null }),
		);
		expect(response.status).toBe(200);
		expect(processLarkMessageMock).toHaveBeenCalledWith({
			...VALID_PAYLOAD,
			messageId: null,
		});
	});

	test("returns 400 for an invalid payload", async () => {
		const response = await POST(buildRequest({ connectionId: "" }));
		expect(response.status).toBe(400);
		expect(processLarkMessageMock).not.toHaveBeenCalled();
	});
});
