...
import { DEFAULT_LARK_MODEL } from "./constants";

export interface LarkAgentAction {
	type: string;
}

export interface LarkAgentResult {
	text: string;
	actions: LarkAgentAction[];
}

interface RunLarkAgentParams {
	prompt: string;
...
const SYSTEM_PROMPT = `You are a helpful assistant in Lark (Feishu) for Rox, a platform for managing tasks and running coding agents in workspaces.
...
Guidelines:
- Be concise and clear; Lark messages should be short
...
- Use plain text, not Slack-specific formatting
...
export function formatActionsForLark(actions: LarkAgentAction[]): string {
	if (actions.length === 0) return "";
	return `Changes:\n${actions.map((action) => `- ${action.type}`).join("\n")}`;
}

export async function formatErrorForLark(error: unknown): Promise<string> {
...
export async function runLarkAgent(
	params: RunLarkAgentParams,
): Promise<LarkAgentResult> {
	const anthropic = new Anthropic({ apiKey: env.ANTHROPIC_API_KEY });
	const actions: LarkAgentAction[] = [];
	let roxMcp: Client | null = null;
	let cleanupRox: (() => Promise<void>) | null = null;

	try {
		const roxMcpResult = await createLarkMcpClient({
...
${agentContext}`;
		...
		const text = await runMcpAgentLoop({
			anthropic,
			roxMcp,
			model: params.model ?? DEFAULT_LARK_MODEL,
			system,
			tools,
			messages,
			...
			onToolLimit: (lastText) => {
				...
				const limitMessage =
					"I stopped after reaching the Lark agent tool limit. Please narrow the request or ask me to continue with a smaller step.";
