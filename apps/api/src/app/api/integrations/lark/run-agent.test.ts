...
const { formatActionsForLark, formatErrorForLark, runLarkAgent } = await import(
	"./run-agent"
);
...
describe("runLarkAgent", () => {
	...
	test("formats temporary provider errors without a secondary model call", async () => {
		const text = await formatErrorForLark(new Error("rate limit exceeded"));

		expect(text).toContain("temporarily unavailable");
		expect(createMessageMock).not.toHaveBeenCalled();
	});

	test("summarizes recorded actions for the user", () => {
		expect(formatActionsForLark([])).toBe("");
		expect(formatActionsForLark([{ type: "create_task" }])).toBe(
			"Changes:\n- create_task",
		);
	});
