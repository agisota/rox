...
const { formatErrorForDiscord, runDiscordAgent } = await import("./run-agent");
...
describe("runDiscordAgent", () => {
	...
	test("returns the assistant text and cleans up the MCP client", async () => {
		messageQueue = [textResponse("Here is your answer")];
	...
	test("formats temporary provider errors without a secondary model call", async () => {
		const text = await formatErrorForDiscord(new Error("rate limit exceeded"));
