...
import { DEFAULT_DISCORD_MODEL } from "./constants";

export interface DiscordAgentAction {
	type: string;
}

export interface DiscordAgentResult {
	text: string;
	actions: DiscordAgentAction[];
}

interface RunDiscordAgentParams {
	prompt: string;
...
const SYSTEM_PROMPT = `You are a helpful assistant in Discord for Rox, a platform for managing tasks and running coding agents in workspaces.
...
Guidelines:
- Be concise and clear; a Discord reply should be readable at a glance
...
async function createDiscordMcpClient({
	organizationId,
	userId,
}: {
	organizationId: string;
...
export function formatActionsForDiscord(actions: DiscordAgentAction[]): string {
	if (actions.length === 0) return "";
	return `Changes:\n${actions.map((action) => `- ${action.type}`).join("\n")}`;
}

export async function formatErrorForDiscord(error: unknown): Promise<string> {
...
export async function runDiscordAgent(
	...
	try {
		const roxMcpResult = await createDiscordMcpClient({
...
${agentContext}`;
		...
		const text = await runMcpAgentLoop({
			anthropic,
			roxMcp,
			model: params.model ?? DEFAULT_DISCORD_MODEL,
			system,
			tools,
			messages,
			progressStatus: TOOL_PROGRESS_STATUS,
			logTag: "discord-agent",
			...
			onToolLimit: (lastText) => {
				...
				const limitMessage =
					"I stopped after reaching the Discord agent tool limit. Please narrow the request or ask me to continue with a smaller step.";
