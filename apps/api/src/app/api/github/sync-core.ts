...
type GithubInstallation = typeof githubInstallations.$inferSelect;

/**
 * Synchronizes repositories and recent pull requests for a single GitHub App
 * installation: upserts accessible repos, paginates PRs updated in the last 30
 * days, reduces each PR's check runs to a single status, and upserts the PRs.
 *
 * Shared by `sync/route.ts` (dev-only, org-keyed) and
 * `jobs/initial-sync/route.ts` (QStash-signed). Callers own auth and response
 * shaping; this core performs only the sync work and returns the repo count.
 */
export async function syncInstallationRepos({
	...
	logPrefix: string;
}): Promise<{ repositoriesCount: number }> {
	...
	return { repositoriesCount: repos.length };
}
