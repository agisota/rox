...
/**
 * The Drizzle-backed XMPP `emitToUnifiedInbox` must write a `comms_participants`
 ...
 * drops every published XMPP event and the thread never surfaces via
 * `comms.listThreads/getThread`. This mirrors the email path's `FIX 1`.
 *
 * A resolvable sender counterpart (a known rox `xmpp` address in
 * `comms_addresses`) is added as a participant too, so an internal rox→rox XMPP
 * message threads for BOTH parties. The participant write is idempotent on the
	...
	return {
		values(vals: AnyRow | AnyRow[]) {
			...
			if (kind === "message") {
				...
				state.insertedOffline.push(row);
			} else {
...
const { createXmppIngestDb } = await import("./drizzleDb");

const baseArgs = {
	organizationId: "org-1",
	toUserId: "user-recipient",
	fromJid: "bob@external.org",
	toJid: "alice@xmpp.rox.one",
...
describe("createXmppIngestDb.emitToUnifiedInbox participants", () => {
	...
	test("adds a resolvable rox sender counterpart as a participant too", async () => {
		...
		state.selectQueue = [
			[], // thread by dedup → none → create
			[{ userId: "user-sender" }], // sender xmpp-address → known rox user
		];
	...
	test("idempotent on repeat: a reply into an existing thread re-writes the participant with onConflictDoNothing", async () => {
		...
		state.selectQueue = [
			...
			[], // sender xmpp-address lookup → external
		];
		await db.emitToUnifiedInbox({ ...baseArgs, stanzaId: "stanza-2" });
