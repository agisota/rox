/**
 * Single-use nonce replay guard for the XMPP bridge ingress (D4).
 *
 * Each signed bridge POST to `/api/xmpp/inbound` carries a one-time nonce; a
 * replayed request reuses it. We reject any nonce we have already seen inside
 ...
 * `xmpp_nonces`, and an `onConflictDoNothing` that returns no row means the
 * nonce was already seen => replay. Replay protection therefore holds across
 ...
 * the authority. Mirrors `lib/mail/nonceStore.ts` (D3).
 */

import { db as defaultDb } from "@rox/db/client";
import { xmppNonces } from "@rox/db/schema";
...
export function createDbNonceStore(
	...
	async function prune(now: number): Promise<void> {
		try {
			await db
				.delete(xmppNonces)
				.where(lt(xmppNonces.expiresAt, new Date(now)));
		} catch {
		...
		async checkAndRecord(nonce, now = Date.now()) {
			...
			try {
				const inserted = await db
					...
					.onConflictDoNothing({ target: xmppNonces.nonce })
					.returning({ nonce: xmppNonces.nonce });
