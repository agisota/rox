...
export function DownloadButton({
	...
	const sizeClasses =
		size === "sm" ? "h-9 px-4 text-sm" : "h-11 px-6 text-sm sm:text-base";

	const buttonClasses = `bg-brand/10 text-[#ff8c3a] border border-brand/20 ${sizeClasses} rounded-full font-normal hover:bg-brand/15 hover:border-brand/35 transition-colors inline-flex items-center justify-center gap-2 whitespace-nowrap ${className}`;
		...
		const sections: DropdownSection[] = [
			...
			},
			{
				...
				items: [
					{
						id: "waitlist",
						label: "Записаться в лист ожидания для Windows и Linux",
					...
					},
					{
						...
						label: "Собрать из исходников на GitHub",
						icon: githubIcon,
	...
	if (isMacPlatform(platform) || platform === Platform.Unknown) {
		return (
			<button
				...
				onClick={goToInterstitial}
			>
				<span className="hidden sm:inline">Скачать для macOS</span>
