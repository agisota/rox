---
title: "Working with Git Worktrees in Rox"
description: "How Rox uses Git worktrees to run multiple AI coding agents in parallel without conflicts. A practical guide to the workflow that lets you 10x your throughput."
author: kiet
date: 2026-02-18
...
Rox solves this with Git worktrees. Every agent gets its own branch and its own copy of the working directory, all backed by the same Git object store. No duplication, no conflicts, no chaos. This post walks through how it works in practice.
...
Here's what happens when you create a new task in Rox:

### 1. Create a Task

You describe what you want done — "add input validation to the signup form" or "refactor the payment service to use the new API." Rox creates a new Git worktree and checks out a fresh branch.
...
When the agent finishes, Rox shows you a diff of everything it changed. The built-in diff viewer highlights additions, deletions, and modifications across all files. You can also open the worktree in your editor (VS Code, Cursor, JetBrains, Xcode) to inspect changes in full context.
...
### How Rox Manages Concurrency

Rox's persistent daemon manages all agent sessions via Unix domain sockets. This architecture means:

- **Sessions survive crashes** — close and reopen Rox, your agents are still running
...
Different agents have different strengths. Rox is agent-agnostic — use whatever works best for the job:
...
When an agent finishes, don't re-read the entire file. Focus on the diff. Rox's built-in diff viewer shows exactly what changed. If a change touches files you didn't expect, that's worth investigating. If it's scoped to what you asked for, a quick review is usually sufficient.

### Clean Up Finished Worktrees

After merging, remove the worktree. Rox handles this automatically when you close a completed task, but it's good practice to keep your worktree list clean. Old worktrees don't affect performance, but they clutter the list.
...
The other limitation is the one-branch rule: Git doesn't allow the same branch to be checked out in multiple worktrees simultaneously. This is actually a feature — it prevents the exact class of bugs that parallel work creates. Rox enforces unique branches per task by default.

---

## Getting Started

If you're already using a CLI coding agent, adding Rox takes about two minutes:

1. **Download Rox** from [rox.one](https://rox.one)
2. **Open your project** — Rox detects your git repository automatically
