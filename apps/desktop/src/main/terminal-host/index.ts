 ...
 * - Socket: ~/.rox/terminal-host.sock
 * - Auth token: ~/.rox/terminal-host.token
 */
...
// ROX_DIR_NAME is imported from shared/constants for multi-worktree support
// This allows workspace-specific home directories (e.g., ~/.rox-my-feature)
const ROX_HOME_DIR = join(homedir(), ROX_DIR_NAME);

// Socket and token paths
const SOCKET_PATH = join(ROX_HOME_DIR, "terminal-host.sock");
const TOKEN_PATH = join(ROX_HOME_DIR, "terminal-host.token");
const PID_PATH = join(ROX_HOME_DIR, "terminal-host.pid");
...
async function startServer(): Promise<void> {
	// Ensure rox directory exists with proper permissions
	if (!existsSync(ROX_HOME_DIR)) {
		mkdirSync(ROX_HOME_DIR, { recursive: true, mode: 0o700 });
		log("info", `Created directory: ${ROX_HOME_DIR}`);
	}

	// Ensure directory has correct permissions
	try {
		chmodSync(ROX_HOME_DIR, 0o700);
	} catch {
...
async function main() {
	...
	log("info", `Home directory: ${ROX_HOME_DIR}`);
