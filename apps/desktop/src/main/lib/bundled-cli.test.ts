...
describe("bundled CLI", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(path.join(tmpdir(), "rox-bundled-cli-"));
		binDir = path.join(tempDir, "bin");
		bundledCliPath = path.join(tempDir, "resources", "bin", "rox");
	...
	it("uses the platform-specific binary and shim names", () => {
		expect(getBundledCliBinaryName("darwin")).toBe("rox");
		expect(getBundledCliShimName("darwin")).toBe("rox");
		expect(getBundledCliBinaryName("win32")).toBe("rox.exe");
		expect(getBundledCliShimName("win32")).toBe("rox.cmd");
	});

	it("builds a POSIX shim that execs the bundled binary safely", () => {
		const cliPath =
			"/Applications/Rox Test.app/Contents/Resources/bin/super'set";
		const shim = buildBundledCliShim(cliPath, "darwin");

		expect(shim).toContain(BUNDLED_CLI_SHIM_MARKER);
		expect(shim).toContain(
			`exec '/Applications/Rox Test.app/Contents/Resources/bin/super'"'"'set' "$@"`,
		);
	});
	...
	it("installs an executable managed shim into the terminal bin directory", () => {
		...
		const shimPath = path.join(binDir, "rox");
	...
	it("does not overwrite an unmanaged rox executable", () => {
		const shimPath = path.join(binDir, "rox");
		mkdirSync(binDir, { recursive: true });
	...
	it("returns missing when the bundled binary is unavailable", () => {
		const status = installBundledCliShim({
			binDir,
			bundledCliPath: path.join(tempDir, "missing", "rox"),
			platform: "darwin",
		});
