...
/**
 * Shell snippet to save all ROX_* env vars before sourcing user RC files.
 * Used in tandem with {@link ROX_ENV_RESTORE} to prevent user shell
 * configs from overriding Rox-managed environment variables (e.g.
 * ROX_WORKSPACE_NAME).
 *
 * @see https://github.com/AidenIO/rox/issues/2386
 */
const ROX_ENV_SAVE = `_rox_saved_env="$(export -p 2>/dev/null | grep ' ROX_')"`;

/**
 * Shell snippet to restore previously saved ROX_* env vars after
 * sourcing user RC files.
 */
const ROX_ENV_RESTORE = `eval "$_rox_saved_env" 2>/dev/null || true`;
...
function buildManagedCommandPrelude(shellName: string, binDir: string): string {
	if (shellName === "fish") {
		...
		return ROX_MANAGED_BINARIES.map(
			(name: RoxManagedBinary) =>
				`functions -q ${name}; and functions -e ${name}
function ${name}
  set -l _rox_wrapper "${escapedBinDir}/${name}"
  if test -x "$_rox_wrapper"; and not test -d "$_rox_wrapper"
    "$_rox_wrapper" $argv
  else
    command ${name} $argv
  end
...
${name}() {
  _rox_wrapper=${quoteShellLiteral(`${binDir}/${name}`)}
  if [ -x "$_rox_wrapper" ] && [ ! -d "$_rox_wrapper" ]; then
    "$_rox_wrapper" "$@"
  else
    command ${name} "$@"
  fi
...
${ROX_ENV_SAVE}
_rox_home="\${ROX_ORIG_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_rox_home"
[[ -f "$_rox_home/.zshenv" ]] && source "$_rox_home/.zshenv"
${ROX_ENV_RESTORE}
...
_rox_home="\${ROX_ORIG_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_rox_home"
[[ -f "$_rox_home/.zprofile" ]] && source "$_rox_home/.zprofile"
${ROX_ENV_RESTORE}
export ZDOTDIR=${quotedZshDir}
`;
...
_rox_home="\${ROX_ORIG_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_rox_home"
[[ -f "$_rox_home/.zshrc" ]] && source "$_rox_home/.zshrc"
${ROX_ENV_RESTORE}
...
if [[ -o interactive ]]; then
  [[ -f "$_rox_home/.zlogin" ]] && source "$_rox_home/.zlogin"
fi
${ROX_ENV_RESTORE}
${buildZshPrecmdHook(paths.BIN_DIR)}
${buildPathPrependFunction(paths.BIN_DIR)}
...
# rox of markers; daemons that only get restarted on protocol bumps
...
__rox_prompt_mark() {
  printf "\\033]777;rox-shell-ready\\007\\033]133;A\\007"
}
# Keep our hook LAST so it fires after direnv and other precmd hooks complete.
precmd_functions=(\${precmd_functions[@]} __rox_prompt_mark)
export ZDOTDIR="$_rox_home"
`;
...
if [[ "$(declare -p PROMPT_COMMAND 2>/dev/null)" == "declare -a"* ]]; then
  PROMPT_COMMAND=("\${PROMPT_COMMAND[@]}" "__rox_prompt_mark")
else
  _rox_orig_prompt_cmd="\${PROMPT_COMMAND}"
  if [[ -n "\${_rox_orig_prompt_cmd}" ]]; then
    PROMPT_COMMAND="\${_rox_orig_prompt_cmd};__rox_prompt_mark"
  else
    PROMPT_COMMAND="__rox_prompt_mark"
  fi
fi
`;
...
export function getShellEnv(
	...
	if (shellName === "zsh") {
		return {
			ROX_ORIG_ZDOTDIR: process.env.ZDOTDIR || os.homedir(),
