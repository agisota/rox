...
const COPILOT_HOOK_SIGNATURE = "# Rox copilot hook";
...
# Auto-inject Rox notification hooks when running inside a v2 Rox terminal.
if [ -n "$ROX_TERMINAL_ID" ] && [ -f "${hookScriptPath}" ]; then
  COPILOT_HOOKS_DIR=".github/hooks"
  COPILOT_HOOK_FILE="$COPILOT_HOOKS_DIR/rox-notify.json"
  ...
  if [ -d ".git/info" ]; then
    grep -qF ".github/hooks/rox-notify.json" ".git/info/exclude" 2>/dev/null || \\
      printf '%s\\n' ".github/hooks/rox-notify.json" >> ".git/info/exclude" 2>/dev/null
  fi
fi
