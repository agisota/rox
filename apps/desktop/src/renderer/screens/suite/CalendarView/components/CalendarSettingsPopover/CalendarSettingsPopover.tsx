...
/**
 * Per-calendar settings popover (NEW). Lets a writer rename/recolor/re-timezone a
 * calendar via `updateCalendar`, and (owner only) delete it via `deleteCalendar`
 * or manage ACL sharing via `shareCalendar`/`unshareCalendar` + `listShares`.
 * The mutation payload is built by the shared `@rox/shared/calendar-share` core
 * so web/desktop/mobile stay in lockstep; `listCalendars` is invalidated by
 * `useCalendarActions` after every write so the cache-first views refresh.
 */
export function CalendarSettingsPopover({
	...
	// Reset the draft whenever the popover (re)opens or the calendar changes so a
	// stale edit never leaks across calendars.
	useEffect(() => {
		if (open) {
		...
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent align="end" className="w-80 space-y-4">
				<div className="space-y-3">
					...
					<div className="space-y-1.5">
						<Label htmlFor="cal-tz" className="text-xs">
							Часовой пояс
						</Label>
						<Input
							id="cal-tz"
