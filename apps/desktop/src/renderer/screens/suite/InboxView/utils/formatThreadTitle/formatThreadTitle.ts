/**
 * Derive a human title for a comms inbox thread row.
 *
 * The comms hub stores `subject` as nullable (chat threads often have none), so
 * the list needs a deterministic, network-free fallback. Mirrors the web inbox
 * util so the desktop chat tab labels threads identically.
 */
