/**
 * Shared attachment-upload helper for the unified ChatPaneShell.
 *
 * Canonical copy of the per-tree `uploadFiles` utils (which are byte-identical
 * and depend only on the shared `apiTrpcClient` + dev-mode singletons, never on
 * a tree-specific tRPC client). Fetches each attachment URL, converts to a data
 * URL, and (outside dev mode) uploads via the shared chat attachment endpoint.
 */
