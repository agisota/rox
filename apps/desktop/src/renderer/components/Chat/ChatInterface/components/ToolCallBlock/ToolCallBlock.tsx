...
import { AnimatedDiffReveal, ToolCardMotion } from "@rox/ui/motion";
...
export function ToolCallBlock({
	...
	const renderExecuteCommand = (): ReactNode => {
	...
	const renderWriteFile = (): ReactNode => {
	...
	const renderEditFile = (): ReactNode => {
	...
	const renderWebSearch = (): ReactNode => {
	...
	const renderWebFetch = (): ReactNode => {
	...
	// Exact tool-name → renderer. One entry per tool; same output as the
	// previous sequential `if (toolName === ...)` chain. Order-independent
	// because keys are exact and non-overlapping; predicate/prefix cases
	// (web_search prefix, READ_ONLY_TOOLS) are handled around this lookup.
	const TOOL_RENDERERS: Record<string, () => ReactNode> = {
		mastra_workspace_execute_command: renderExecuteCommand,
		mastra_workspace_write_file: renderWriteFile,
		mastra_workspace_edit_file: renderEditFile,
		ast_smart_edit: renderEditFile,
		...
		create_task: () => <CreateTaskToolCall part={part} />,
		update_task: () => <UpdateTaskToolCall part={part} />,
		list_tasks: () => <ListTasksToolCall part={part} />,
		get_task: () => <GetTaskToolCall part={part} />,
		delete_task: () => <DeleteTaskToolCall part={part} />,
		list_task_statuses: () => <ListTaskStatusesToolCall part={part} />,
		list_members: () => <ListMembersToolCall part={part} />,
		list_devices: () => <ListDevicesToolCall part={part} />,
		list_workspaces: () => <ListWorkspacesToolCall part={part} />,
		list_projects: () => <ListProjectsToolCall part={part} />,
		get_app_context: () => <GetAppContextToolCall part={part} />,
		get_workspace_details: () => <GetWorkspaceDetailsToolCall part={part} />,
		create_workspace: () => <CreateWorkspaceToolCall part={part} />,
		switch_workspace: () => <SwitchWorkspaceToolCall part={part} />,
		update_workspace: () => <UpdateWorkspaceToolCall part={part} />,
		delete_workspace: () => <DeleteWorkspaceToolCall part={part} />,
		start_agent_session: () => <StartAgentSessionToolCall part={part} />,
		...
		lsp_inspect: () => <LspInspectToolCall part={part} />,
		task_write: () => <TaskWriteToolCall part={part} />,
		task_check: () => <RoxToolCall part={part} toolName="Update task status" />,
		submit_plan: () => <RoxToolCall part={part} toolName="Submit plan" />,
		subagent: () => (
			<SubagentToolCall
				part={part}
				args={args}
		...
		skill: renderSkill,
		load_skill: renderSkill,
	};

	const renderToolContent = (): ReactNode => {
		const exactRenderer = TOOL_RENDERERS[toolName];
		if (exactRenderer) {
			return exactRenderer();
		}

		// --- Web search prefix fallback (e.g. provider-prefixed web_search) ---
