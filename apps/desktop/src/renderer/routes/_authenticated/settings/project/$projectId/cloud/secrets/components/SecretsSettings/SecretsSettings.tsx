			...
			<div className="space-y-6">
				{isConnected && organizationId && project.neonProjectId ? (
					...
					<SettingsSection
						...
						description="Свяжите проект с облачным проектом для sandbox и переменных окружения."
					>
						<div className="flex items-center justify-between">
							<p className="text-sm text-muted-foreground">
								{linkToNeon.isPending
									? "Подключение..."
									: "Облачный проект не подключен."}
							</p>
							{!linkToNeon.isPending && (
								<Button
									size="sm"
								...
								>
									{isCreatingCloud ? "Подключение..." : "Подключить к Cloud"}
								</Button>
							)}
						</div>
					</SettingsSection>
