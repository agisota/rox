...
- [~] M5+ — Product-surface bindings PR-03..14 (one milestone per surface; see roadmap). (completed: PR-12 run button, PR-13 Settings motion preference, two AnimatedNumber count bindings; remaining: bespoke color reimplementations + hot surfaces)
  ...
  - [x] (2026-06-09) PR-05 flavor — workspace sidebar +/- diff counts bound to `AnimatedNumber` (`DashboardSidebarWorkspaceDiffStats`). Color-agnostic, additive, product colors preserved. Commit `2337266`.
  - [x] (2026-06-09) PR-09 flavor — workspace sidebar tab **badge counts** bound to `AnimatedNumber` (`SidebarHeader`); rounds before formatting to keep the `99+` cap. Commit `d2c3143`.
  - [x] (2026-06-09) PR-11 — shared `StatusIndicator` (working/permission/review dots: workspace icon, V2 notifications, group strip) now gates its looping `animate-ping` halo on `useMotionPreference().reduced`. Default appearance unchanged; reduced/off motion drops the loop while the solid dot stays. Commit `d69f167`. Then the `ScriptsEditor` "Saving…" amber `animate-pulse` dot, same contract — commit `77b36bc`. Remaining one-off pulses left as-is (lower value): v1 `WorkspaceRow` red ping, `RemoteWorkspacesDemo` (paywall), and `animate-pulse` skeleton loaders (skeletons arguably essential loading feedback).
  - [x] (2026-06-09) PR-08 — Changes-tab toolbar (`ChangesToolbar`) total +/− now springs via `AnimatedNumber` as the agent edits files. Kept the product `text-green-400`/`text-red-400` and the `+`/`-` signs; singular header (not per-row, not virtualized). Commit pending.

  **Hard constraint discovered (governs every color binding).** All MONAD *primitives* (`DeltaField`, `ValidatorGate`, `EventParticle`, `TargetAttractor`, …) resolve `var(--monad-*)` tokens that `tokens.css` scopes under `[data-monad-root]` ("MONAD only paints inside `MonadThemeProvider`"). Dropping a raw primitive onto a product surface → undefined colors/fonts → broken. So on real surfaces we **never** mount the raw primitive; we replicate its look with **product** color tokens + the color-agnostic motion helpers (`AnimatedNumber`, `useMotionPreference`). The raw primitives stay in the `/monad` gallery, where `data-monad-root` is in scope. This is why PR-05/08/09/11 above are "-style" bindings, not primitive drops.
  - Deferred deliberately: PR-07 (chat tool-call lifecycle) and PR-08 (terminal runtime frame) animate hot/virtualized surfaces — forbidden by the Phase B governing rule; not done blind. PR-05 full `DeltaField`, PR-06 `ValidatorGate`, PR-09 `TargetAttractor`, PR-10 `EventParticle`, PR-11 `StatusPulse` are bespoke color reimplementations (MONAD primitives use `var(--monad-*)`, which only resolves under `[data-monad-root]`), so each needs product colors hand-mapped — higher visual risk without a render. Approached only the color-agnostic motion helpers (AnimatedNumber) blind, per the user's explicit "proceed blind, at my risk" decision.
