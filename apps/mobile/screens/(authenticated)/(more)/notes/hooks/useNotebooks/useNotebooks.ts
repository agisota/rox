...
export type Notebook = RouterOutputs["notebooks"]["listNotebooks"][number];
...
export function useNotebooks(): UseNotebooksResult {
	...
	const load = useCallback(async () => {
		setError(null);
		try {
			const result = await apiClient.notebooks.listNotebooks.query();
			setNotebooks(result);
		} catch (err) {
	...
	const createNotebook = useCallback(
		async (name: string) => {
			...
			setCreating(true);
			try {
				await apiClient.notebooks.createNotebook.mutate({ name: trimmed });
				await load();
				return true;
			} catch (err) {
				setError(
