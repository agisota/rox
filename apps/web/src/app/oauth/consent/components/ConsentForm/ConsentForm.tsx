...
> = {
	openid: {
		label: "Подтверждать вашу личность",
		icon: <LuShieldCheck className="size-4" />,
	},
	profile: {
		label: "Получать доступ к данным профиля (имя, изображение)",
		icon: <LuUser className="size-4" />,
	},
	email: {
		label: "Получать доступ к вашему адресу электронной почты",
		icon: <LuMail className="size-4" />,
	},
	offline_access: {
		label: "Сохранять подключение (refresh tokens)",
...
export function ConsentForm({
	...
	const handleConsent = async (accept: boolean) => {
		...
		try {
			...
			setError(getConsentErrorMessage(err));
			setIsLoading(false);
		}
	};
		...
		<div className="mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[400px]">
			<div className="flex flex-col space-y-2 text-center">
				...
				<p className="text-muted-foreground text-sm">
					...
					запрашивает доступ к вашему аккаунту Rox
				</p>
			</div>
			...
			<div className="bg-muted/50 rounded-lg border p-4">
				...
				{showOrgPicker ? (
					<div className="mb-4">
						...
						<p className="text-muted-foreground mt-1.5 text-xs">
							Это приложение получит доступ к данным выбранной организации.
						</p>
					</div>
				) : selectedOrg ? (
					<p className="text-muted-foreground mb-3 text-sm">
				...
				<p className="mb-2 text-sm font-medium">Это приложение сможет:</p>
				<ul className="space-y-2">
					...
					<li className="flex items-center gap-2 text-sm">
						...
						<span>Получать доступ к данным вашей организации</span>
					</li>
				</ul>
			</div>
			...
			<div className="flex gap-3">
				...
				>
					{isLoading ? "Авторизация..." : "Разрешить"}
				</Button>
			</div>

			<p className="text-muted-foreground px-8 text-center text-xs">
				Разрешая доступ, вы позволяете этому приложению использовать ваши данные
				в соответствии с его условиями обслуживания и политикой
				конфиденциальности.
			</p>
		</div>
	);
...
function getConsentErrorMessage(err: unknown): string {
	if (!(err instanceof Error) || !err.message) {
		return "Произошла ошибка";
	}

	if (err.message === "Не удалось выбрать организацию") {
		return err.message;
	}

	if (err.message === "Не удалось обработать разрешение") {
		return err.message;
	}
