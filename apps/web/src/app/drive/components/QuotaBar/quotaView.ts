/**
 * Pure presentation math for the Drive quota bar (used / cap). Kept separate
 * from the component so the percentage + warning thresholds are unit-testable
 * without a DOM.
 *
 * Mirrors the server's soft-meter model (DQ2): going over the cap is possible
 * with overage opt-in, so the percentage is clamped to 100 for the bar but the
 * raw `overBytes` is surfaced for an over-quota notice.
 */
