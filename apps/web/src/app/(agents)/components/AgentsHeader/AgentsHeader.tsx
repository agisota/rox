...
export function AgentsHeader() {
	...
	const handleSignOut = async () => {
		try {
			...
			handleActionError("Не удалось выйти. Попробуйте еще раз.", error);
			return false;
		}
	};
	...
	const handleSwitchOrganization = async (organizationId: string) => {
		...
		try {
			...
			handleActionError(
				"Не удалось сменить организацию. Попробуйте еще раз.",
				error,
			);
			return false;
		}
			...
			<div className="mx-auto flex h-12 w-full items-center justify-between px-4">
				...
				>
					...
					<svg
						...
						className="hidden h-4 w-auto text-foreground"
						aria-label={COMPANY.NAME}
					>
