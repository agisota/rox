...
/**
 * FIX 3 — an `email`-transport SSE event must refresh the Mail tab (mail.*), and
 * an in-app event must refresh the chat surface (comms.*). The open thread is
 * only refreshed when the event targets it AND the open tab matches the event's
...
function makeClient() {
	...
	const queryClient = {
		invalidateQueries: mock((opts: { queryKey: unknown }) => {
...
describe("applyCommsStreamEvent (FIX 3)", () => {
