 ...
 * without rendering a form. Web mirror of the desktop `fieldSections` so both
 * surfaces render the registry's typed sections identically (#594, CLAUDE.md #4
 * cross-platform parity).
 *
 * Pattern provenance (for future contributors):
 * - The categorized, searchable palette (`NodePalette`) borrows the sim.ai
 *   block-palette pattern: category headers + per-category accent + search.
 ...
 * - The data-driven, registry-generated auto-form itself follows the dify.ai
 *   pattern. We borrow PATTERNS from simstudioai/sim (Apache-2.0), not code.
 */
