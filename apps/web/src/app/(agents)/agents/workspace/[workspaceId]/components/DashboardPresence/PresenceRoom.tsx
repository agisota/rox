...
export interface PresenceRoomProps {
	/** Org/project-scoped room id (`org:{orgId}:dashboard:{dashboardId}`). */
	roomId: string;
	/**
	 * Mints a scoped LiveBlocks session token. In the app this wraps the tRPC
	 * `collab.authRoom` mutation so the client never holds `LIVEBLOCKS_SECRET_KEY`.
	 */
	authEndpoint: (roomId: string) => Promise<{ token: string }>;
	className?: string;
	/**
	 * Injectable LiveBlocks bindings (defaults to the real `@rox/collab/client`).
...
/**
 * The live half of the dashboard presence mount: opens the LiveBlocks room and
 * feeds the other peers' presence into the shared `PresenceStack`. Mounted only
 * once the gate is open (`DashboardPresence`), so it always has a valid room id
 * and a configured LiveBlocks key.
 */
