...
/**
 * Connect/Edit dialog for an Agent-Native source. Drives the pure
 * {@link SourceFormState} mapping and dispatches `agentSource.create` /
 * `agentSource.update`. Credentials entered here are PLAINTEXT and travel up to
 * the router, which AES-encrypts them into `encryptedConfig`; the form never
 * reads or echoes the ciphertext (the `list`/`get` projection omits it), so on
 * edit the credential rows start blank and only re-send when the user re-enters
 * them. Writes are org-admin gated server-side — a non-admin's submit surfaces
 * the router's FORBIDDEN as an inline toast.
 */
