...
/**
 * Connect-a-source management surface: a header with a "Подключить источник"
 * action, the {@link SourcesList} (kind/status + lifecycle transitions), and the
 * {@link SourceForm} create/edit dialog. The active organization comes from the
 * session — the same source of truth the composer's `useAgentControls` uses — so
 * every list/create/update/setStatus call is org-scoped and the org-admin gate
 * on writes applies. When there is no active org the surface explains why instead
 * of issuing org-less queries.
 */
