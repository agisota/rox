...
export default function WorkspacesPage() {
	...
	return (
		<div className="mx-auto min-h-[100dvh] max-w-3xl px-5 py-8">
			<h1 className="text-xl font-medium">Воркспейсы</h1>
			{organizationName && (
			...
			<section className="mt-6 rounded-lg border p-4">
				<h2 className="text-sm font-medium">Новый воркспейс</h2>
				...
				>
					{creating ? "Создаем…" : "Создать воркспейс"}
				</button>
			</section>

			<section className="mt-6">
				<h2 className="text-sm font-medium">Ваши воркспейсы</h2>
				<div className="mt-3 flex flex-wrap items-center gap-2">
					<input
						value={search}
						onChange={(event) => setSearch(event.target.value)}
						placeholder="Искать воркспейсы…"
				...
				{workspaces === null ? (
					...
					<p className="text-muted-foreground mt-3 text-sm">
						{workspaces.length === 0
							...
							: "Нет воркспейсов по выбранным фильтрам."}
					</p>
				) : (
							...
							<li key={workspace.id}>
								...
								>
									...
									<div className="text-muted-foreground mt-0.5 text-xs">
										{workspace.projectName} · {workspace.branch} · создан{" "}
