...
step_load_env() {
  ...
  if [ -z "${ROX_ROOT_PATH:-}" ]; then
    error "ROX_ROOT_PATH not set"
    return 1
  fi

  if [ ! -f "$ROX_ROOT_PATH/.env" ]; then
    error "Root .env file not found at $ROX_ROOT_PATH/.env"
    return 1
  fi

  set -a
  # shellcheck source=/dev/null
  source "$ROX_ROOT_PATH/.env"
  set +a
...
step_setup_neon_branch() {
  ...
  WORKSPACE_NAME="${ROX_WORKSPACE_NAME:-$(basename "$PWD")}"
...
step_prepare_electric() {
  ...
  ELECTRIC_CONTAINER=$(echo "rox-electric-$container_suffix" | cut -c1-64)
  ELECTRIC_SECRET="${ELECTRIC_SECRET:-local_electric_dev_secret}"

  # Step 7 allocates ROX_PORT_BASE; Electric must use that reserved port.
  if [ -z "${ROX_PORT_BASE:-}" ]; then
    error "ROX_PORT_BASE not set before preparing Electric"
    return 1
  fi

  ELECTRIC_PORT=$((ROX_PORT_BASE + 9))
...
ROX_RESERVED_PORTS="3659 4045 5000 5060 5061 6000 6566 6665 6666 6667 6668 6669 6697 7000"
...
port_base_is_safe() {
  ...
  local range=$2
  local reserved
  for reserved in $ROX_RESERVED_PORTS; do
...
allocate_port_base() {
  local alloc_file="$HOME/.rox/port-allocations.json"
  local lock_dir="$HOME/.rox/port-allocations.lock"
  local start=3000
  local range=20
  ...
  if [ -n "$existing" ]; then
    if port_base_is_safe "$existing" "$range"; then
      export ROX_PORT_BASE="$existing"
      release_port_alloc_lock "$lock_dir"
      return 0
    fi
    echo "  Existing port base $existing overlaps a reserved port (${ROX_RESERVED_PORTS}); reallocating..."
  ...
  export ROX_PORT_BASE="$candidate"
  release_port_alloc_lock "$lock_dir"
  return 0
}
...
step_write_env() {
  ...
  if [ -z "${ROX_ROOT_PATH:-}" ] || [ ! -f "$ROX_ROOT_PATH/.env" ]; then
    error "Root .env file not available"
    return 1
  fi

  # Copy root .env
  if ! cp "$ROX_ROOT_PATH/.env" .env; then
  ...
  # Append workspace-specific values
  {
    ...
    write_env_var "ROX_WORKSPACE_NAME" "${WORKSPACE_NAME:-$(basename "$PWD")}"
    write_env_var "ROX_HOME_DIR" "$PWD/rox-dev-data"
    echo ""
    ...
    echo ""
    echo "# Workspace Ports (allocated from ROX_PORT_BASE=$BASE, range=20)"
    write_env_var "ROX_PORT_BASE" "$BASE"
    ...
    write_env_var "NEXT_PUBLIC_RELAY_URL" "http://localhost:$RELAY_PORT"
    write_env_var "ROX_WEB_URL" "http://localhost:$WEB_PORT"
    echo ""
  ...
  # Generate .rox/ports.json for static port name mapping in the desktop app
  local rox_dir
  rox_dir="${ROX_SCRIPT_DIR:-$(dirname "$0")}"
  cat > "$rox_dir/ports.json" <<PORTSJSON
{
  "ports": [
...
PORTSJSON
  success "Port name mapping written to .rox/ports.json"
...
step_setup_local_mcp() {
  ...
  local api_port="${API_PORT:-$((${ROX_PORT_BASE:-3000} + 1))}"
  local local_url="http://localhost:${api_port}/api/agent/mcp"

  # Add or update rox-local entry
  local tmp_file="${mcp_file}.tmp.$$"
  if ! jq --arg url "$local_url" '.mcpServers["rox-local"] = {"type": "http", "url": $url}' "$mcp_file" > "$tmp_file"; then
...
step_seed_auth_token() {
  echo "🔑 Seeding auth token into rox-dev-data/..."

  local source_token="$HOME/.rox/auth-token.enc"
  local dev_data_dir="rox-dev-data"
...
step_seed_host_dbs() {
  echo "🛰️  Seeding host-service DBs into rox-dev-data/host/..."

  local source_root="$HOME/.rox/host"
  local dev_data_dir="rox-dev-data"
...
step_seed_local_db() {
  echo "💾 Seeding local DB into rox-dev-data/..."

  local source_db="$HOME/.rox/local.db"
  local dev_data_dir="rox-dev-data"
