...
- Never touch the **production** database directly. Run all schema changes against a dedicated Neon branch first.
- Schema in `packages/db/src/`
- Use Drizzle ORM for all database operations

## DB migrations
- Spin up a new Neon branch for migrations and point the root `.env` at it locally while iterating.
- Change the Drizzle schema in `packages/db/src/schema/`, then generate the migration with `bunx drizzle-kit generate --name="<name_snake_case>"`.
- Running migrations is allowed: apply generated migrations to the Neon branch (e.g. `bunx drizzle-kit migrate` / `drizzle-kit push`) and verify the tables before promoting to production.
- `NEON_ORG_ID` and `NEON_PROJECT_ID` env vars are set in `.env`; use `neonctl` for branch/connection management.
- Migration files under `packages/db/drizzle/` are auto-generated by Drizzle — let `drizzle-kit generate` produce them rather than hand-editing.
