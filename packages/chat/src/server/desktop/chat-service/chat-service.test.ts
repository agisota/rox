...
let testRoxHomeDir: string | null = null;
...
describe("ChatService OpenAI auth storage", () => {
	beforeEach(() => {
		...
		testRoxHomeDir = mkdtempSync(join(tmpdir(), "chat-service-test-"));
		process.env.ROX_HOME_DIR = testRoxHomeDir;
	...
	afterEach(() => {
		if (testRoxHomeDir) {
			rmSync(testRoxHomeDir, { recursive: true, force: true });
			testRoxHomeDir = null;
		}
		if (originalRoxHomeDir) {
