...
// tools other than Rox can also `git worktree add`, and their
...
async function startHostEnsure(
	...
	const { getHostId, getHostName } = await import("@rox/shared/host-info");
				...
				try {
					...
					if (existing) {
						...
						if (localOid !== null && !adoptLocalBranch) {
							...
							throw new TRPCError({
								code: "CONFLICT",
								message: `Local branch "${resolvedBranch}" exists outside Rox and points at a different commit than PR #${input.pr} (local ${localOid.slice(0, 7)}, PR ${prMetadata.headRefOid.slice(0, 7)}). ${cleanupHint}`,
							});
						}
