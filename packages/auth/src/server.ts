...
import { VerifyEmail } from "@rox/email/emails/verify-email";
...
export const auth = betterAuth({
	...
	emailAndPassword: {
		// Real cloud auth: email + password sign-up/sign-in is available in every
		// environment, not just local dev. Verification is enforced in deployed
		// environments; local dev skips it so the quick "Local Admin" login works.
		enabled: true,
		autoSignIn: true,
		minPasswordLength: 8,
	...
	emailVerification: {
		...
		sendVerificationEmail: async ({ user, url }) => {
			await resend.emails.send({
				...
				to: user.email,
				subject: "Verify your email for Rox",
					...
					if (domain) {
						...
						for (const org of matchingOrgs) {
							try {
								...
								// addMember may have created the DB record before a downstream error — check
