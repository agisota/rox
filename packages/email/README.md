...
1. Generate PNG assets (email clients don't support SVG):
   ...
   - Footer logo (`logo-full.png`): 512x83px (pixelated ROX text)
   - Social icons: 48x48px (displayed at 24x24 for retina)
2. Place in `apps/marketing/public/assets/emails/`
3. Images load from `https://rox.one/assets/emails/` in production
...
```bash
NEXT_PUBLIC_MARKETING_URL=https://rox.one
```
...
import { WelcomeEmail } from "@rox/email/emails/welcome";
...
await resend.emails.send({
  from: "noreply@rox.one",
  to: user.email,
  subject: "Welcome to Rox!",
  html,
});
```
...
**Example:**
```tsx
<Button href="https://app.rox.one">Get Started</Button>
<Button href="https://rox.one" variant="secondary">
  Learn More
</Button>
```
