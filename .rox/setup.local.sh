...
# Rox stack backed by a local Postgres container + fake credentials — no
...
# shellcheck source=/dev/null
source "$ROX_SCRIPT_DIR/lib/setup/steps.sh" # reuse allocate_port_base + helpers
...
local_allocate_ports() {
  ...
  LOCAL_DB_PROJECT="rox-$(sanitize_name "${ROX_WORKSPACE_NAME:-$(basename "$PWD")}")"
...
local_write_env() {
  ...
  if [ -z "${ROX_PORT_BASE:-}" ] || [ -z "$LOCAL_NEON_PROXY_PORT" ]; then
...

  {
    ...
    write_env_var "ROX_WORKSPACE_NAME" "${ROX_WORKSPACE_NAME:-$(basename "$PWD")}"
...
DEVVARS
  ...
  cat > "$ROX_SCRIPT_DIR/ports.json" <<PORTSJSON
{
  "ports": [
...
local_write_config_overlay() {
  echo "🔧 Writing .rox/config.local.json (untracked overlay)..."
  cat > "$ROX_SCRIPT_DIR/config.local.json" <<'CONFIGLOCAL'
{
  "setup": ["./.rox/setup.local.sh"],
  "teardown": ["./.rox/teardown.local.sh"]
}
CONFIGLOCAL
...
local_setup_main() {
  ...
  echo "🚀 Setting up Rox for LOCAL development..."
  echo ""
